...
void LteRealisticChannelModel::initialize()
{
   ...
   // statistics
   rcvdSinr_ = cComponent::registerSignal("rcvdSinr");   
}
...
std::vector<double> LteRealisticChannelModel::getSINR(LteAirFrame *frame, UserControlInfo* lteInfo)
{
   ...
   for (unsigned int i = 0; i < band_; i++)
   {
       ...
       EV << "\t ext[" << extCellInterference[i] << "] - multi[" << multiCellInterference[i] << "] - recvPwr["
          << dBmToLinear(snrVector[i]) << "] - sinr[" << dBmToLinear(snrVector[i]-den) << "]\n";
...
bool LteRealisticChannelModel::error_D2D(LteAirFrame *frame, UserControlInfo* lteInfo, const std::vector<double>& rsrpVector)
{
   ...
   double totalPer = per * pow(harqReduction_, nTx - 1);
    ...
    cModule* parent = check_and_cast<omnetpp::cModule*>(this->getParentModule()->getParentModule());

    std::string parent_name = parent->getModuleType()->getFullName();
    
    if ((lteInfo->getDirection() == D2D || lteInfo->getDirection() == D2D_MULTI) && (parent_name.compare("artery.lte.Car")==0)){
        ...
        if(managmentLayer->first_message_LTE){
            managmentLayer->PRR_LTE_first = 1 - totalPer;
            managmentLayer->SINR_LTE_first = sumSnr / usedRBs;
            managmentLayer->first_message_LTE = false;
        }
        managmentLayer->PRR_LTE = 1 - totalPer;
        managmentLayer->SINR_LTE = sumSnr / usedRBs;
    } 
   ...
   if (er <= totalPer)
   {
       ...
       // Signal too weak, we can't receive it
       // std::cout << "Not Received = PRR = " << 1 - totalPer << " SNR = " << sumSnr / usedRBs << "\n";
       return false;
   }
   // std::cout << "Received = PRR = " << 1 - totalPer << " SNR = " << sumSnr / usedRBs << "\n";
   // Signal is strong enough, receive this Signal
