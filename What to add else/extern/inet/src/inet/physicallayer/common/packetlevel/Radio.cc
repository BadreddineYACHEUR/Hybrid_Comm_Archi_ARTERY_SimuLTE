...
// TODO: this would draw twice from the random number generator in isReceptionSuccessful: auto isReceptionSuccessful = medium->isReceptionSuccessful(this, transmission, part);
            ...
            // if (isReceptionSuccessful)
            //     std::cout << "Reception YESSSSSSSSSSSSSSSSSSS" << "\n";
            // else if (! isReceptionSuccessful)
            //     std::cout << "Reception NOOOOOOOOOOOOOOOOOOOO" << "\n";
...
void Radio::sendUp(cPacket *macFrame)
{
    ...
    // updating the SINR and PRR values in the managment layer
    ...
    managmentLayer->SINR_ITS_G5 = indication->getMinSNIR();

    if (!std::isnan(indication->getPacketErrorRate())){
        emit(packetErrorRateSignal, indication->getPacketErrorRate());
        
        if(managmentLayer->first_message_ITS_G5){
            managmentLayer->PRR_ITS_G5_first = indication->getPacketErrorRate();
            managmentLayer->SINR_ITS_G5_first = indication->getMinSNIR();
            managmentLayer->first_message_ITS_G5 = false;
        }
        managmentLayer->PRR_ITS_G5 = indication->getPacketErrorRate();
